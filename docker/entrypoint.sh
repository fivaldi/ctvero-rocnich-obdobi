...
then
    ...
    GNUPGHOME="$(mktemp -d /tmp/.gnupgXXXXXX)" gpg --quiet --batch --yes --decrypt --passphrase="$CTVERO_DEPLOY_PROD_SECRET" --output deploy-prod-files/.env deploy-prod-files/.env.gpg
